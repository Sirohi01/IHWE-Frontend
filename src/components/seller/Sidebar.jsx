...
const navItems = [
    { id: 'overview', label: 'Dashboard', icon: LayoutDashboard, path: '/seller-dashboard' },
    { id: 'directory', label: 'Attendee Directory', icon: Users, path: '/seller-dashboard/directory' },
    { id: 'exhibitors', label: 'Exhibitors', icon: Building2, path: '/seller-dashboard/exhibitors' },
    { id: 'details', label: 'Full Details', icon: ClipboardList, path: '/seller-dashboard/details' },
    { id: 'profile', label: 'My Profile', icon: User, path: '/seller-dashboard/profile' },
    { id: 'payments', label: 'Payment History', icon: CreditCard, path: '/seller-dashboard/payments' },
    { id: 'history', label: 'Activity Logs', icon: History, path: '/seller-dashboard/history' },
    { id: 'help', label: 'Support Hub', icon: HelpCircle, path: '/seller-dashboard/support' },
];
        ...
        <aside className={`fixed top-16 left-0 bottom-0 z-50 bg-white border-r border-slate-200 flex flex-col transition-all duration-300 overflow-hidden print:hidden ${sidebarOpen ? "w-56" : "w-14"}`}>
            <nav className="flex-1 py-4 space-y-0.5 px-2 overflow-y-auto">
                {navItems.map(item => {
                    ...
                    const active = location.pathname === item.path || (item.path === '/seller-dashboard' && location.pathname === '/seller-dashboard/');
