...
function Ticks({ msg }: { msg: any }) {
    if (msg.senderType !== "buyer") return null;
...
export default function BuyerChatTab({ data, inNavbar = false }: Props) {
    ...
    const buyerId = data?._id?.toString() || "";
    const buyerName = data?.companyFirmName || "Buyer";
    ...
    useEffect(() => {
        ...
        s.on("connect", () => {
            setConnected(true);
            s.emit("join_room", { roomId, userId: buyerId, userType: "buyer", userName: buyerName });
            // Mark admin messages as read on open
            s.emit("mark_read", { roomId, readerType: "buyer" });
        });
        ...
        s.on("messages_seen", ({ seenBy }: any) => {
            if (seenBy === "admin") {
                // Admin has seen our messages
                setMessages(prev => prev.map(m => m.senderType === "buyer" ? { ...m, readByAdmin: true } : m));
            }
        });
    ...
    const sendMessage = () => {
        ...
        socket.emit("send_message", {
            roomId, buyerRegistrationId: buyerId, buyerName,
            senderType: "buyer", senderId: buyerId, senderName: buyerName,
            message: input.trim()
        });
        setInput("");
    ...
    const handleTyping = (val: string) => {
        ...
        socket.emit("typing", { roomId, senderType: "buyer", senderName: buyerName });
