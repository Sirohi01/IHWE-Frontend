...
export default function InvoicePrintTemplate({ document, company, bankDetails, settings, headerImageUrl, heading = 'TAX INVOICE', invoiceCopy = 'ORIGINAL INVOICE' }: Props) {
    ...
    return (
        <div className="bg-white border border-slate-300 p-10 text-[11px] font-sans text-black" style={{ fontFamily: 'Calibri, Arial, sans-serif', maxWidth: '1000px', margin: '0 auto', position: 'relative' }}>
            ...
            {(document?.delivery_challans || []).length > 0 && (
                <div style={{ marginBottom: 8 }}>
                    ...
                    {(document.delivery_challans || []).map((challan: any, challanIndex: number) => (
                        <div key={challan.delivery_challan_id || `${challan.challan_no}-${challanIndex}`} style={{ border: '1px solid #ccc', borderTop: 0, padding: 6 }}>
