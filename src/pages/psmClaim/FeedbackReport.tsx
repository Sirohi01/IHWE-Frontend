                        ...
                        <tbody>
                            {[
                                ...
                                <tr key={row.id} className="border-b border-black">
                                    <td className="w-10 text-center border-r border-black font-bold py-2">{row.id}</td>
                                    <td className="p-2 border-r border-black font-bold w-1/2">{row.label}</td>
                                    <td className="p-2 bg-slate-50/30 print:bg-transparent">
                                        <textarea
                                            value={row.value}
                                            onChange={(e) => setFormData({ ...formData, [row.key]: e.target.value })}
                                            className="w-full bg-transparent outline-none resize-none min-h-[1.5rem] print:hidden"
                                            rows={1}
                                        />
                                        <div className="hidden print:block whitespace-pre-wrap break-words min-h-[1.5rem]">
                            ...
                            <tr className="border-b border-black">
                                ...
                                <td className="p-2 border-r border-black font-bold">
                                    ...
                                    <p className="mt-8 font-normal text-[10px]">[about 200 words along with photographs of event]</p>
                                </td>
                                <td className="p-2">
                                    <textarea
                                        value={formData.comments}
                                        onChange={(e) => setFormData({ ...formData, comments: e.target.value })}
                                        className="w-full bg-transparent outline-none resize-none min-h-[6rem] print:hidden"
                                        placeholder="Type your comments here..."
                                    />
                                    <div className="hidden print:block whitespace-pre-wrap break-words min-h-[6rem]">
                            ...
                            {[
                                ...
                                <tr key={row.id} className="border-b border-black">
                                    ...
                                    <td className="p-2 border-r border-black font-bold">{row.label}</td>
                                    <td className="p-2">
                                        <textarea
                                            value={row.value}
                                            onChange={(e) => setFormData({ ...formData, [row.key]: e.target.value })}
                            ...
                            <tr className="border-b border-black">
                                ...
                                <td colSpan={2} className="p-2 font-bold leading-tight">
                                    ...
                                    <div className="mt-3 overflow-hidden">
                                        <table id="tech-table" className="w-full border-collapse border border-black text-[10px]">
                                            <thead>
                                            ...
                                            <tbody>
                                                {formData.technologies.map((tech, idx) => (
                                                    <tr key={idx}>
                                                        <td className="border border-black p-0"><input type="text" value={tech.country} onChange={(e) => updateTech(idx, 'country', e.target.value)} className="w-full p-1 bg-transparent outline-none" /></td>
                                                        <td className="border border-black p-0"><input type="text" value={tech.field} onChange={(e) => updateTech(idx, 'field', e.target.value)} className="w-full p-1 bg-transparent outline-none" /></td>
                                                        <td className="border border-black p-0"><input type="text" value={tech.description} onChange={(e) => updateTech(idx, 'description', e.target.value)} className="w-full p-1 bg-transparent outline-none" /></td>
                                                        <td className="border border-black p-0"><input type="text" value={tech.contact} onChange={(e) => updateTech(idx, 'contact', e.target.value)} className="w-full p-1 bg-transparent outline-none" /></td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                            ...
                            <tr>
                                ...
                                <td className="font-bold border-b border-black p-2">Remarks/Suggestions, if any</td>
                                <td className="p-2 border-b border-black">
                                    <textarea
                                        value={formData.remarks}
                                        onChange={(e) => setFormData({ ...formData, remarks: e.target.value })}
                                        className="w-full bg-transparent outline-none resize-none min-h-[3rem] print:hidden"
                                    />
                                    <div className="hidden print:block whitespace-pre-wrap break-words min-h-[3rem]">
                        ...
                        <div className="flex justify-between items-end mt-12 px-2">
                            <div className="flex gap-2 items-end">
                                ...
                                <input
                                    ...
                                    className="border-b border-black w-32 px-1 bg-transparent outline-none font-bold"
                                />
                            </div>
                            <div className="flex flex-col items-center">
                                <div className="border-b-2 border-black w-64"></div>
                                <span className="font-bold text-[10px] mt-1 text-center uppercase">SIGNATURE/NAME/DESIGNATION OF PARTICIPANT</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <style dangerouslySetInnerHTML={{
                ...
                @media print {
                    ...
                    * {
                    ...
                    /* Keep internal grid for point 14 table */
