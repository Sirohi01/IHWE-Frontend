...
export default function Overview() {
    ...
    const [stats, setStats] = useState({ totalSellers: 0, completedPayments: 0 });
    ...
    useEffect(() => {
        ...
        const fetchProfile = async () => {
            if (currentSeller?.id) {
                try {
                    const response = await sellerApi.getProfile(currentSeller.id);
                    if (response.data.success) {
    ...
    if (!currentSeller || !profile) return null;
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 pb-4 border-b">
                    <div>
                        ...
                        <p className="text-[10px] text-slate-400 uppercase font-bold tracking-widest font-sans mt-0.5">
                            {profile.companyName} · {profile.registrationCategory}
                        </p>
                    </div>

                    <div className="flex items-center gap-2">
                        <span className={`px-3 py-1 text-[10px] font-black border uppercase tracking-widest font-sans ${profile.paymentStatus === 'Completed'
                ...
                <div className="space-y-6">
                    <Section title="Registration Overview">
                        <InfoGrid rows={[
                            ...
                            ['Category', profile.registrationCategory],
                            ['Payment Status', profile.paymentStatus],
                            ['Registration Date', profile.createdAt ? new Date(profile.createdAt).toLocaleDateString() : '—'],
                            ['Member Since', '2026 Edition'],
                            ['Primary Product', profile.primaryProductCategory],
                        ]} />
                    </Section>
                    ...
                    <Section title="Professional Profile">
                        <InfoGrid rows={[
                            ...
                            ['Firm Name', profile.companyFirmName],
                            ['Nature of Bus.', profile.natureOfBusiness],
                            ['Ann. Turnover', profile.annualTurnover],
                        ]} />
                    </Section>

                    <Section title="Selling Preferences & Capacity">
                        <InfoGrid rows={[
                            ['Selling Frequency', profile.sellingFrequency],
                            ['Annual Sale Vol.', profile.estimatedAnnualSaleValue],
                            ['Target Market', Array.isArray(profile.targetMarket) ? profile.targetMarket.join(', ') : profile.targetMarket],
                            ['Pricing Range', profile.pricingRange],
                            ['Pre-Sch. B2B', profile.requirePreScheduledB2B],
                        ]} />
                    </Section>
                </div>
