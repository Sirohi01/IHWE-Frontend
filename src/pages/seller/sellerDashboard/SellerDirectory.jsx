    ...
    <div className="space-y-6 animate-fadeIn pb-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-1 ml-7">Networking with {sellers.length} registered delegates</p>
        </div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-[#23471d] text-white">
                ...
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest w-40">Seller ID</th>
                  ...
                  >
                    ...
                    <td className="px-6 py-4 min-w-[250px]">
                       <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                             <p className="text-[11px] font-black text-slate-800 tracking-tight truncate">{seller.fullName}</p>
                             <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5 truncate">{seller.companyName}</p>
                          </div>
                       </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                       <div className="flex items-center gap-1.5 text-slate-500">
                          ...
                          <span className="text-[10px] font-bold truncate max-w-[150px]">{seller.city}, {seller.country}</span>
                       </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                       <span className={`px-2 py-0.5 rounded-md text-[8px] font-black uppercase tracking-widest border ${
                          seller.registrationCategory === 'VIP' ? 'bg-amber-50 text-amber-600 border-amber-200' :
                          seller.registrationCategory === 'Hosted' ? 'bg-purple-50 text-purple-600 border-purple-200' :
                          'bg-blue-50 text-blue-600 border-blue-200'
                       }`}>
                          {seller.registrationCategory}
                       </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                       <div className={`flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest ${
                          seller.paymentStatus === 'Completed' ? 'text-emerald-500' : 
                          seller.paymentStatus === 'Pending' ? 'text-amber-500' : 'text-red-500'
                       }`}>
                          {seller.paymentStatus === 'Completed' ? <CheckCircle2 className="w-3 h-3" /> : 
                           seller.paymentStatus === 'Pending' ? <Clock className="w-3 h-3" /> : <AlertCircle className="w-3 h-3" />}
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
           <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">
              Showing {filtered.length} of {sellers.length} delegates registered for IHWE 2026
           </p>
