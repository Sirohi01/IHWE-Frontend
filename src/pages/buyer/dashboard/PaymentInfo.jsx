...
export default function PaymentInfo() {
  ...
  const payments = [
    { 
      id: "PAY-" + (currentBuyer.transactionId?.split('-').pop() || "1001"), 
      date: new Date(currentBuyer.createdAt).toLocaleDateString(), 
      plan: currentBuyer.registrationCategory, 
      amount: "₹" + (currentBuyer.registrationFee || "0"), 
      method: currentBuyer.paymentMode || "N/A", 
      status: currentBuyer.paymentStatus === "Completed" ? "Successful" : currentBuyer.paymentStatus, 
      txnId: currentBuyer.transactionId || "N/A" 
    }
  ];
          ...
          <div className="p-8 relative z-10">
            <div className="flex items-center justify-between mb-8">
              ...
              <span className="px-3 py-1 bg-emerald-500 rounded-full text-[10px] font-black uppercase tracking-widest">
                {currentBuyer.paymentStatus === 'Completed' ? 'Active Plan' : 'Pending'}
              </span>
            </div>
            
            <p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-300/80 mb-2">Current Membership</p>
            <h3 className="text-2xl font-black mb-1 leading-tight">{currentBuyer.registrationCategory}</h3>
            <p className="text-4xl font-black mb-8">₹{currentBuyer.registrationFee || '0'}<span className="text-sm font-bold text-emerald-300/60 ml-2">/ Year</span></p>
            
            <div className="space-y-4 pt-6 border-t border-white/10">
              <div className="flex items-center justify-between text-sm">
                <span className="text-emerald-50/60 font-bold">Registration ID</span>
                <span className="font-black">{currentBuyer.registrationId}</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-emerald-50/60 font-bold">Payment Status</span>
                <span className="font-black">{currentBuyer.paymentStatus}</span>
              </div>
            </div>
          </div>
          ...
          <div className="premium-card p-8 flex flex-col justify-between">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <p className="text-2xl font-black text-slate-800 tracking-tight">₹{currentBuyer.registrationFee || '0'}</p>
              </div>
            </div>
