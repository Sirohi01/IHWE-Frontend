...
function Section({ icon, letter, title, note, children, className = '', headerRight }: SectionProps) {
    return (
...
function StepNode({ step }) {
    ...
    return (
        <div className="relative z-10 flex flex-col items-center gap-2">
            ...
            <small className={`${isDone ? 'text-[#087536]' : 'text-[#8090ad]'} whitespace-nowrap text-[9px] font-bold`}>{step.label}</small>
            <small
...
function StatusBadge({ status }) {
    ...
    return (
        <span className={`inline-flex items-center gap-1 whitespace-nowrap rounded px-1.5 py-1 text-[9px] font-bold ${
            isUploaded ? 'text-[#087536]' : 'text-[#e07a12]'
        }`}>
            {isUploaded ? (
                <span className="grid h-3.5 w-3.5 place-items-center rounded-full bg-[#087536] text-white">
...
function DocumentRow({ index, doc, onUpload }) {
    ...
    return (
        <tr className="border-b border-[#e9eef4] last:border-b-0">
            <td className="whitespace-nowrap p-2.5 text-[9.5px] font-semibold text-[#6b7ea3]">{index + 1}</td>
            <td className="min-w-[150px] p-2.5 text-[9.5px] font-bold text-[#061743]">
            ...
            <td className="min-w-[150px] p-2.5 text-[9.5px] font-medium text-[#5a6c92]">{doc.description}</td>
            <td className="whitespace-nowrap p-2.5">
                <StatusBadge status={doc.status} />
            </td>
            <td className="min-w-[130px] p-2.5 text-[9.5px] font-semibold">
                {isUploaded ? (
                    <span className="inline-flex items-center gap-1.5 whitespace-nowrap">
                        <a href="#" onClick={(event) => event.preventDefault()} className="font-bold text-[#2a6bd6] hover:underline">{doc.file}</a>
                    </span>
                ) : (
            ...
            <td className="whitespace-nowrap p-2.5 text-right">
                {isUploaded ? (
                    <button type="button" className="h-[20px] rounded-md border border-[#cfe4d8] bg-[#eff9f3] px-3 text-[9px] font-bold text-[#087536]">
                        View
                    </button>
                ) : (
                    <button type="button" onClick={() => onUpload(doc.id)} className="h-[20px] rounded-md border border-[#dadff2] bg-[#eef0fb] px-3 text-[9px] font-bold text-[#4c5aa8]">
                        Upload
                    </button>
                )}
            </td>
...
function AutoDocCard({ title }) {
    return (
        <div className="flex min-w-0 items-center gap-2.5 rounded-lg border border-[#e9eef4] bg-[#fbfcfe] p-2.5">
            <span className=" text-[#087536]">
                <FileText size={25} strokeWidth={1.8} />
            </span>
            <div className="min-w-0">
                <strong className="block text-[9.5px] font-bold text-[#061743]">{title}</strong>
                <a href="#" onClick={(event) => event.preventDefault()} className="mt-0.5 inline-flex items-center gap-1 text-[9px] font-bold  hover:underline">
                    View / Download
...
export default function MSMEPMSDocumentsUpload({ data, onBack, onContinue }) {
            ...
            <header className="mb-1 flex flex-wrap items-start justify-between gap-4">
                <div>
                    ...
                    <p className="text-[13px] font-bold text-[#061743]">
                        <b className="font-extrabold text-[#087536]">Step 3 of 5</b> — Documents Upload
                    </p>
                </div>
                ...
                <div className="flex min-w-0 flex-col gap-4">
                    ...
                    <main className="flex flex-col gap-3.5">
                        <Section
                            ...
                            icon={<UploadCloud size={17} strokeWidth={1.8} />}
                            headerRight={
    ...
    <div className="flex flex-col gap-0.5">
        ...
        <div className="flex items-baseline gap-1">
            <b className="text-[20px] font-extrabold leading-none text-[#087536]">
    ...
    <span className="h-1.5 w-40 overflow-hidden rounded-full bg-[#e1e5ed]">
        ...
        />
                        ...
                        >
                            <p className="mb-3 text-[9.5px] font-medium text-[#5a6c92]">Please upload clear and valid documents to proceed with verification.</p>

                            <div className="overflow-x-auto rounded-lg border border-[#e9eef4]">
                                <table className="w-full min-w-[720px] border-collapse">
                                    <thead>
                                        <tr className="bg-[#f6f9fc]">
                                            <th className="whitespace-nowrap p-2.5 text-left text-[9.5px] font-bold text-[#061743]">#</th>
                                            <th className="whitespace-nowrap p-2.5 text-left text-[9.5px] font-bold text-[#061743]">Document Name</th>
                                            <th className="whitespace-nowrap p-2.5 text-left text-[9.5px] font-bold text-[#061743]">Description</th>
                                            <th className="whitespace-nowrap p-2.5 text-left text-[9.5px] font-bold text-[#061743]">Status</th>
                                            <th className="whitespace-nowrap p-2.5 text-left text-[9.5px] font-bold text-[#061743]">File / Preview</th>
                                            <th className="whitespace-nowrap p-2.5 text-left text-[9.5px] font-bold text-[#061743]"></th>
                                            <th className="whitespace-nowrap p-2.5 text-right text-[9.5px] font-bold text-[#061743]">Action</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                        ...
                        <footer className="grid grid-cols-1 items-center gap-3.5 rounded-lg border border-[#dbe4ef] bg-white p-2 sm:grid-cols-[120px_minmax(0,1fr)_190px]">
                            ...
                            <button
                                ...
                                className={`flex h-7 items-center justify-center gap-2 rounded-md text-[10px] font-bold text-white shadow-[0_4px_9px_rgba(8,117,54,0.18)] ${
                ...
                <aside className="flex min-w-0 flex-col gap-3.5">
                    ...
                    <section className="min-w-0 overflow-hidden rounded-xl border border-[#dbe4ef] bg-white px-3 py-2">
                        <h2 className="mb-3 text-[13px] font-extrabold text-[#061743]">Upload Progress</h2>
                        <div className="flex items-center gap-3">
                            <ProgressRing percent={percentComplete} />
                            <div>
                                <strong className="block text-[13px] font-extrabold text-[#061743]">{uploadedCount} of {totalCount}</strong>
                                <p className="mt-1 text-[9px] font-medium text-[#31446c]">Documents Uploaded</p>
                                <p className="mt-1.5 text-[9px] font-bold text-[#e07a12]">{pendingCount} Pending</p>
                            </div>
                        </div>
                        <a href="#" onClick={(event) => event.preventDefault()} className="mt-3 flex items-center gap-1 border-t border-[#e9eef4] pt-3 text-[9.5px] font-bold text-[#2a6bd6] hover:underline">
