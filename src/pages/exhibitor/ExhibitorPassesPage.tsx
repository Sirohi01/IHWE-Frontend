...
import {
    Users, UserCheck, Car, Wrench,
    ArrowRight, Check, Sparkles,
    X,
    CheckCircle2, ChevronRight, Hash, Info, Calendar, Bell
} from "lucide-react";
...
export default function ExhibitorPassesPage() {
    ...
    // Top summary cards need totals split by complimentary vs paid — derived from
    // each pass's already-computed complimentary/used/remaining/totalQuota fields
    // (no new backend data needed).
    const aggregateStats = passes.reduce((acc, p) => {
        const complimentary = Number(p.complimentary || 0);
        const totalQuota = Number(p.totalQuota || 0);
        const used = Number(p.used || 0);
        const remaining = Number(p.remaining || 0);
        const complimentaryRemaining = Number(p.complimentaryRemaining || 0);
        const complimentaryUsed = Math.max(complimentary - complimentaryRemaining, 0);
        const totalRequested = totalQuota - remaining;
        const paidPurchased = Math.max(totalRequested - complimentary, 0);
        const paidUsed = Math.max(used - complimentaryUsed, 0);
        const paidRemaining = Math.max(paidPurchased - paidUsed, 0);
        acc.totalAll += totalQuota;
        acc.totalComplimentary += complimentary;
        acc.totalExtraCapacity += Math.max(totalQuota - complimentary, 0);
        acc.usedAll += used;
        acc.remainingAll += remaining;
        acc.complimentaryUsed += complimentaryUsed;
        acc.complimentaryRemaining += complimentaryRemaining;
        acc.paidPurchased += paidPurchased;
        acc.paidUsed += paidUsed;
        acc.paidRemaining += paidRemaining;
        return acc;
    }, {
        totalAll: 0, totalComplimentary: 0, totalExtraCapacity: 0, usedAll: 0, remainingAll: 0,
        complimentaryUsed: 0, complimentaryRemaining: 0, paidPurchased: 0, paidUsed: 0, paidRemaining: 0
    });
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 mb-3">
                <div>
                    <h1 className="text-2xl font-bold text-[#11381e] tracking-tight">Exhibitor Pass</h1>
                    <p className="text-sm text-slate-500 font-semibold mt-1">
                        Manage your complimentary passes and purchase extras for your team.
                    </p>
                </div>
                <div className="flex items-center gap-2.5 bg-blue-50 border border-blue-100 rounded-xl px-3.5 py-2.5 shrink-0">
                    <Info size={16} className="text-blue-500 shrink-0" />
                    <p className="text-[11px] font-bold text-blue-800 max-w-[240px] leading-tight">
                        Pass configuration will be saved and applied only for the selected event.
                    </p>
            ...
            <div className="flex flex-col xl:flex-row gap-2 mb-2">
                <div className="xl:flex-1 bg-[#f7f5ff] border border-[#e6e1fb] rounded-xl p-2.5 text-center">
                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-wide mb-0.5">Total Passes (All)</p>
                    <p className="text-xl font-black text-[#5b21b6] leading-none">{aggregateStats.totalAll}</p>
                    <div className="flex items-center justify-center gap-4 mt-1.5 text-[8.5px] font-bold text-slate-500">
                        <span>From Complimentary<br /><span className="text-emerald-600 font-black text-xs">{aggregateStats.totalComplimentary}</span></span>
                        <span>From Extra (Paid)<br /><span className="text-blue-600 font-black text-xs">{aggregateStats.totalExtraCapacity}</span></span>
                    </div>
                    <div className="mt-1.5 pt-1.5 border-t border-[#e6e1fb] flex items-center justify-center gap-3 text-[8.5px] font-bold text-slate-500">
                        <span>Used: <b className="text-slate-800">{aggregateStats.usedAll}</b></span>
                        <span>Remaining: <b className="text-slate-800">{aggregateStats.remainingAll}</b></span>
                    </div>
                </div>

                <div className="xl:flex-1 bg-[#f2fbf4] border border-[#d7f0dc] rounded-xl p-2.5 text-center">
                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-wide mb-0.5">Total Complimentary Passes</p>
                    <p className="text-xl font-black text-[#15803d] leading-none">{aggregateStats.totalComplimentary}</p>
                    <div className="mt-1.5 flex items-center justify-center gap-3 text-[8.5px] font-bold text-slate-500">
                        <span>Used: <b className="text-slate-800">{aggregateStats.complimentaryUsed}</b></span>
                        <span>Remaining: <b className="text-slate-800">{aggregateStats.complimentaryRemaining}</b></span>
                    </div>
                </div>

                <div className="xl:flex-1 bg-[#f0f7ff] border border-[#dbe9fb] rounded-xl p-2.5 text-center">
                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-wide mb-0.5">Total Extra (Paid) Passes</p>
                    <p className="text-xl font-black text-[#1a3a7c] leading-none">{aggregateStats.paidPurchased}</p>
                    <div className="mt-1.5 flex flex-wrap items-center justify-center gap-x-3 gap-y-0.5 text-[8.5px] font-bold text-slate-500">
                        <span>Purchased: <b className="text-slate-800">{aggregateStats.paidPurchased}</b></span>
                        <span>Used: <b className="text-slate-800">{aggregateStats.paidUsed}</b></span>
                        <span>Remaining: <b className="text-slate-800">{aggregateStats.paidRemaining}</b></span>
                    </div>
                </div>

                <div className="xl:flex-[2] grid grid-cols-5 gap-1.5">
                    {passes.map((pass) => {
                        const isVehicle = pass.id === 'vehicle';
                        const complimentary = isVehicle ? pass.complimentaryTwoWheeler + pass.complimentaryFourWheeler : pass.complimentary;
                        const remaining = isVehicle ? pass.remainingTwoWheeler + pass.remainingFourWheeler : pass.remaining;
                        return (
                            <div key={pass.id} className={`rounded-xl border ${pass.themeClasses.border} ${pass.themeClasses.bg} p-2 flex flex-col items-center text-center`}>
                                <span className="text-[7px] font-black uppercase tracking-wide text-slate-500 leading-tight">{pass.title}</span>
                                <span className={`text-lg font-black ${pass.themeClasses.text} mt-0.5 leading-none`}>{complimentary}</span>
                                <span className="text-[7.5px] font-bold text-slate-500 mt-1">Used: {pass.used}</span>
                                <span className="text-[7.5px] font-bold text-slate-500">Rem: {remaining}</span>
                            </div>
                        );
                    })}
                </div>
            ...
            {/* ─── YOUR COMPLIMENTARY ENTITLEMENT + DAILY ENTITLEMENT ─── */}
            <div className="grid grid-cols-1 xl:grid-cols-[2fr_1fr] gap-2 mb-2">
                <div className="bg-white border border-slate-100 rounded-2xl shadow-sm p-4">
                    <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
                        <h3 className="text-[13px] font-black text-[#15803d] uppercase tracking-wide">
                            Your Complimentary Entitlement <span className="text-slate-400 font-bold normal-case text-xs">(Use First)</span>
                        </h3>
                        <span className="text-[10px] font-black bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-full px-2.5 py-1">
                            Total Complimentary Passes: {aggregateStats.totalComplimentary}
                        </span>
                    </div>
                    <div className="grid grid-cols-5 gap-2 text-center">
                        {passes.map((pass) => {
                            ...
                            return (
                                <div key={pass.id}>
                                    <p className={`text-3xl font-black ${pass.themeClasses.text} leading-none`}>{complimentary}</p>
                                    <p className={`text-[11px] font-black ${pass.themeClasses.text} mt-1.5`}>{pass.title}</p>
                                    <p className="text-[9px] text-slate-400 font-bold mt-0.5">{pass.subtitle}</p>
                                </div>
                            );
                        })}
                    </div>
                    <div className="mt-4 bg-emerald-50/60 border border-emerald-100 rounded-lg py-2 px-3 flex items-center gap-2">
                        <CheckCircle2 size={14} className="text-emerald-600 shrink-0" />
                        <p className="text-[11px] font-bold text-emerald-800">
                            Use complimentary quota first. Extra passes, lunch and water bottles can be purchased online.
                        </p>
                    </div>
                </div>

                <div className="bg-white border border-slate-100 rounded-2xl shadow-sm p-3 flex flex-col justify-between">
                    <div>
                        <h3 className="text-[13px] font-black text-[#11381e] mb-1.5 uppercase">
                            Daily Entitlement <span className="text-slate-400 font-bold text-xs normal-case">(for 3 days)</span>
                        </h3>
                        <div className="divide-y divide-slate-100">
                            <div className="flex items-center justify-between py-1.5">
                                <div className="flex items-center gap-2">
                                    <img loading="lazy" decoding="async" src={thaliImg} alt="Packed Thali Lunch" className="w-7 h-7 object-contain shrink-0" />
                                    <span className="text-xs font-bold text-slate-700">Packed Thali Lunch</span>
                                </div>
                                <div className="text-right shrink-0">
                                    <p className="text-base font-black text-emerald-600 leading-none">{passConfigs.find(c => c.passType === 'lunch')?.complimentaryQuota ?? 2}</p>
                                    <p className="text-[8px] text-slate-400 font-bold uppercase mt-0.5">Persons Per Day</p>
                                </div>
                            </div>
                            <div className="flex items-center justify-between py-1.5">
                                <div className="flex items-center gap-2">
                                    <img loading="lazy" decoding="async" src={bottleImg} alt="Water Bottle" className="w-7 h-7 object-contain shrink-0" />
                                    <span className="text-xs font-bold text-slate-700">Water Bottle</span>
                                </div>
                                <div className="text-right shrink-0">
                                    <p className="text-base font-black text-blue-600 leading-none">{passConfigs.find(c => c.passType === 'water')?.complimentaryQuota ?? 2}</p>
                                    <p className="text-[8px] text-slate-400 font-bold uppercase mt-0.5">Bottles Per Day</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="mt-1.5 bg-emerald-50 border border-emerald-100 rounded-lg py-1.5 text-center">
                        <span className="text-[10px] font-black text-emerald-700">Provided for ALL 3 DAYS (21 – 23 August 2026)</span>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-2 mb-2">
                ...
                <div className="bg-white p-3 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between">
                    <div>
                        <h3 className="font-black text-[13px] text-[#11381e] mb-2.5 uppercase">How It Works</h3>
                        <div className="flex items-start">
                            {[
                                { n: 1, title: 'Apply Online', desc: 'Select the pass type', bg: 'bg-emerald-600' },
                                { n: 2, title: 'Make Payment', desc: 'Secure online payment', bg: 'bg-orange-500' },
                                { n: 3, title: 'Get Approved', desc: 'Quick verification & approval', bg: 'bg-emerald-600' },
                                { n: 4, title: 'Receive Pass', desc: 'Instant digital pass on email', bg: 'bg-purple-600' },
                            ].map((step, i, arr) => (
                                <div key={step.n} className="flex items-center flex-1">
                                    <div className="flex flex-col items-center text-center flex-1 px-1">
                                        <div className={`w-7 h-7 rounded-full ${step.bg} text-white flex items-center justify-center font-black text-xs shadow-sm shrink-0`}>
                                            {step.n}
                                        </div>
                                        <span className="text-[10px] font-black text-slate-800 mt-1 leading-tight">{step.title}</span>
                                        <span className="text-[8.5px] text-slate-400 font-bold mt-0.5 leading-tight">{step.desc}</span>
                                    </div>
                                    {i < arr.length - 1 && (
                                        <div className="border-t-2 border-dashed border-slate-200 flex-1 mb-8" />
                                    )}
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="bg-[#f4fbf6] border border-[#d2edd9] rounded-lg p-2 flex items-start gap-1.5 mt-2.5">
                        <div className="w-3.5 h-3.5 rounded-full bg-emerald-500 text-white flex items-center justify-center flex-shrink-0 mt-0.5">
                            <Check size={9} strokeWidth={3.5} />
                        </div>
                        <p className="text-[10px] text-[#15803d] font-bold leading-tight">
                ...
                <div className="bg-white p-3 rounded-2xl border border-slate-100 shadow-sm">
                    <div className="flex items-center gap-2 mb-2.5">
                        <Bell size={16} className="text-amber-500 shrink-0" strokeWidth={2.5} />
                        <h3 className="font-black text-[13px] text-[#11381e] uppercase">Important Notes</h3>
                    </div>
                        ...
                        <div className="space-y-2 flex-1">
                            {notes.map((note, i) => (
                                <div key={i} className="flex items-start gap-2">
                                    ...
                                    <span className="text-[11px] text-slate-700 font-extrabold leading-tight">
                                        {note}
                                    </span>
                                </div>
                            ))}
                        </div>
                        <div className="w-[80px] shrink-0 flex items-center justify-center self-center">
            ...
            {/* ─── NEED EXTRA PASSES? PURCHASE INSTANTLY ONLINE ─── */}
            <div className="bg-white border border-slate-100 rounded-2xl shadow-sm p-4 mb-2">
                <h3 className="text-[13px] font-black text-[#11381e] uppercase">Need Extra Passes? Purchase Instantly Online</h3>
                <p className="text-xs text-slate-500 font-semibold mt-0.5 mb-3">
                    After complimentary quota is used, you can purchase extra passes for your team.
                </p>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
                    {passes.map((pass) => (
                        <div key={pass.id} className={`rounded-xl border ${pass.themeClasses.border} ${pass.themeClasses.bg} p-3 flex flex-col justify-between text-center`}>
                            <div>
                                <p className="text-xs font-black text-slate-800">{pass.title}</p>
                                <p className="text-[9px] text-slate-500 font-bold mt-0.5">{pass.subtitle}</p>
                                {pass.id === 'vehicle' ? (
                                    <div className="grid grid-cols-2 divide-x divide-slate-200/80 mt-2">
                                        <div className="px-1">
                                            <p className="text-[9px] font-bold text-slate-500 leading-tight">2-Wheeler</p>
                                            <p className="text-[11px] font-black text-slate-800 mt-1">₹{pass.priceTwoWheeler} / Extra</p>
                                        </div>
                                        <div className="px-1">
                                            <p className="text-[9px] font-bold text-slate-500 leading-tight">Car / 4-Wheeler</p>
                                            <p className="text-[11px] font-black text-slate-800 mt-1">₹{pass.priceFourWheeler} / Extra</p>
                                        </div>
                                    </div>
                                ) : pass.id === 'delegate' ? (
                                    <p className="text-sm font-black text-slate-800 mt-2">On Request</p>
                                ) : (
                                    <>
                                        <p className="text-xl font-black text-slate-800 mt-2 leading-none">₹{pass.price}</p>
                                        <p className="text-[9px] text-slate-400 font-bold mt-0.5">Per Pass</p>
                                    </>
                                )}
                            </div>
                            <button
                                onClick={() => pass.id === 'delegate' ? setIsDelegateModalOpen(true) : handleOpenModal(pass)}
                                className={`mt-3 w-full h-7 rounded-lg text-[10px] font-black flex items-center justify-center gap-1 shadow-sm hover:shadow-md transition-all ${pass.themeClasses.btnBg}`}
                            >
                                Purchase Extra <ArrowRight size={11} />
                            </button>
                        </div>
                    ))}

                    <div className="rounded-xl border border-blue-100 bg-blue-50/40 p-3 flex flex-col justify-between text-center">
                        <div>
                            <p className="text-xs font-black text-slate-800">Need Extra</p>
                            <p className="text-xs font-black text-slate-800">Lunch or Water?</p>
                        ...
                        <button
                            ...
                            className="mt-3 w-full h-7 rounded-lg text-[10px] font-black flex items-center justify-center gap-1 bg-blue-600 hover:bg-blue-700 text-white shadow-sm hover:shadow-md transition-all"
                        >
                            Purchase Now <ArrowRight size={11} />
                        </button>
                    </div>
                </div>
            </div>
